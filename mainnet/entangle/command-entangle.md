...
```bash
entangled q bank balances mywallet_public_address --home ${HOME}/.entangled --chain-id entangle_33033-1 --node tcp://localhost:16708
```
...
```
curl -sS http://localhost:16708/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}'
```
...
```bash
entangled tx staking delegate $(entangled keys show wallet --bech val -a) 1000000engl \
--from mywallet \
--gas-adjustment 1.4 \
--home ${HOME}/.entangled \
--node tcp://localhost:16708 \
...
```bash
entangled tx staking delegate entvaloper1yxhagvehavfjwtaar8a25vxwp349fauzsekyxn 1000000engl \ 
--from mywallet \
--gas-adjustment 1.4 \ 
--gas auto \
--home ${HOME}/.entangled \
...
```bash
entangled tx staking redelegate $(entangled keys show wallet --bech val -a) <TO_VALOPER_ADDRESS> 1000000engl \
--from mywallet 
--gas-adjustment 1.4 \
--gas auto \
--home ${HOME}/.entangled \
...
```bash
entangled tx staking unbond $(entangled keys show wallet --bech val -a) 1000000engl \
--from mywallet \
--gas-adjustment 1.4 \
--gas auto \
--home ${HOME}/.entangled \
...
```
entangled tx bank send wallet <TO_WALLET_ADDRESS> 1000000engl \
--from mywallet \
--gas-adjustment 1.4 \
--gas auto \
--home ${HOME}/.entangled \
